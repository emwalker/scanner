...
#[cfg(test)]
mod tests {
    use super::super::{AudioTask, DeviceEnumerationTask, ScanBandTask, Task, TaskScheduler};
    ...
    fn test_device_enumeration_task_mock() {
        ...
        while let Ok(event) = discovery_rx.recv_timeout(Duration::from_millis(10)) {
            match event {
                crate::discovery::Event::Added(device_info) => {
                    device_count += 1;
    ...
    fn test_parallel_tasks() {
        ...
        if status.len() == 2 {
            ...
            if let (Some(scan), Some(audio)) = (scan_status, audio_status) {
                if let (Some(scan_tuner), Some(audio_tuner)) = (&scan.tuner_id, &audio.tuner_id) {
