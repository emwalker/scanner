    ...
    for (window_id, window) in displayable_windows.iter() {
        ...
        let candidates = if use_selectable {
            ...
            window
                ...
                .into_iter()
                .filter(|c| c.status != crate::ui::tui::model::CandidateStatus::Rejected)
                .collect::<Vec<_>>()
        } else {
    ...
    let line = if let Some(audio_quality) = &ctx.candidate.audio_quality {
            ...
            || ctx.candidate.status == CandidateStatus::Rejected
        {
            ...
            let (quality_text, quality_style) = if ctx.is_playing {
                ...
                (
                    match audio_quality {
                        crate::audio::quality::AudioQuality::Good => ctx.theme.quality_good_text(),
                        crate::audio::quality::AudioQuality::Moderate => {
                            ctx.theme.quality_moderate_text()
                        }
                        crate::audio::quality::AudioQuality::Poor => ctx.theme.quality_poor_text(),
