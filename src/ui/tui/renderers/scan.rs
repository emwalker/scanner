    ...
    for (window_id, window) in displayable_windows.iter() {
        ...
        for candidate in displayable {
            ...
            let should_highlight = match &model.ui_mode {
                crate::ui::tui::model::UiMode::AwaitingTune { tuning_index, .. } => {
                    current_candidate_index == *tuning_index
                }
                crate::ui::tui::model::UiMode::Listening { playing_index, .. } => {
...
struct CandidateLineContext<'a> {
    candidate: &'a crate::ui::tui::model::CandidateProgress,
    is_selected: bool,
...
fn render_candidate_line(ctx: &CandidateLineContext) -> Line<'static> {
    ...
    let base_style = if ctx.should_highlight {
        Style::default()
            .bg(ctx.theme.active_highlight_bg())
            .add_modifier(Modifier::BOLD)
    } else {
        Style::default()
    };
    ...
    if let Some(quality) = &ctx.candidate.audio_quality {
        ...
        spans.push(Span::styled(
            ...
            base_style.fg(if ctx.should_highlight {
                ctx.theme.active_highlight_quality()
            } else {
                quality_color
            }),
        ));
