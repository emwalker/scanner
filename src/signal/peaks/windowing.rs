...
pub fn apply_window(samples: &mut [f32], window_type: &crate::core::types::WindowType) {
    let n = samples.len();
    if n <= 1 {
    ...
    let rustradio_window_type = match window_type {
        crate::core::types::WindowType::Rectangular => {
            // Rectangular window: no-op (multiply by 1.0)
            return;
        }
        crate::core::types::WindowType::Hamming => rustradio::window::WindowType::Hamming,
        crate::core::types::WindowType::Hanning => {
        ...
        crate::core::types::WindowType::BlackmanHarris => {
            rustradio::window::WindowType::BlackmanHarris
        }
    };
...
mod tests {
    ...
    fn test_fft_processing_maintains_speed() {
        ...
        let _baseline_peaks =
            crate::signal::peaks::collect_peaks_from_source(&config, &mut baseline_generator)
        ...
        let _preprocessed_peaks =
            crate::signal::peaks::collect_peaks_from_source(&config, &mut preprocessed_generator)
    ...
    fn measure_spectral_leakage_reduction(
        no_window_peaks: &[crate::core::types::Peak],
        windowed_peaks: &[crate::core::types::Peak],
        main_frequency: f64,
    ) -> f32 {
