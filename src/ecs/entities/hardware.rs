use crate::ecs::{
    Entity, HardwareConnectionComponent, HardwareInfoComponent, HardwareLifecycleComponent,
};
use crate::hardware::{Capabilities, DeviceId, DeviceTrait, types::Backend};
use std::sync::{Arc, Mutex};

/// Entity representing a physical SDR hardware device
///
/// Unlike TunerEntity which represents individual tuner channels,
/// HardwareEntity represents the device itself (which may have multiple tuners).
pub struct HardwareEntity {
    id: DeviceId,
    pub info: HardwareInfoComponent,
    pub connection: HardwareConnectionComponent,
    pub lifecycle: HardwareLifecycleComponent,
}

impl HardwareEntity {
    /// Create a new hardware entity for a connected device
    pub fn new(
        device_id: DeviceId,
        label: String,
        ...
        Self {
            id: device_id.clone(),
            info: HardwareInfoComponent::new(device_id, label, capabilities),
            connection: HardwareConnectionComponent::new_connected(device_handle),
            lifecycle: HardwareLifecycleComponent::new(backend, num_tuners),
        }
    }

    /// Create a hardware entity without device handle (metadata only)
    pub fn new_metadata_only(
        ...
        Self {
            id: device_id.clone(),
            info: HardwareInfoComponent::new(device_id, label, capabilities),
            connection: HardwareConnectionComponent::new_connected(None),
            lifecycle: HardwareLifecycleComponent::new(backend, num_tuners),
        }
    }
...
impl Entity for HardwareEntity {
    type Id = DeviceId;
...
impl std::fmt::Debug for HardwareEntity {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HardwareEntity")
            .field("id", &self.id)
...
mod tests {
    ...
    fn test_hardware_entity_creation() {
        ...
        let entity = HardwareEntity::new_metadata_only(
            device_id.clone(),
