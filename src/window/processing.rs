...
use crate::window::config::WindowMetadata;
use std::sync::Arc;
use std::thread;
...
pub(super) fn peaks(
    ...
    if station_mode {
        ...
        crate::fm::collect_peaks(config, sdr_rx_for_peaks, center_freq)
    }
}
...
pub(super) fn candidates_from_peaks(
    ...
    for candidate in crate::fm::find_candidates(peaks, config, center_freq) {
