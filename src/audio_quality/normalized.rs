...
impl AudioQualityMetrics {
    ...
    fn calculate_quality_score(
        ...
        let loudness_score = ((integrated_loudness_lufs + 50.0) / 50.0).clamp(0.0, 1.0);
        let flatness_score = (1.0 - pcen_spectral_flatness).clamp(0.0, 1.0); // Lower flatness = better music
        let snr_score = (normalized_snr_db / 30.0).clamp(0.0, 1.0);
        let stability_score = temporal_stability.clamp(0.0, 1.0);

        // Weighted combination based on importance for FM audio quality
